...
from experiments.models import Experiment, Group, Participant, EEGData, \
    EMGData, EEGSetting, EMGSetting, TMSData, TMSSetting, AdditionalData, \
    ContextTree, GenericDataCollectionData, GoalkeeperGameData, Step, \
    QuestionnaireResponse, Questionnaire, EEG, EMG, TMS, GoalkeeperGame, \
    Stimulus, EMGElectrodeSetting, EEGElectrodePosition, \
...
classification_of_disease_fields = [
    ...
    {
        "field": 'abbreviated_description',
        "header": 'abbreviated_description',
        "description": _("Abbreviated description")
    },
]
...
def save_to_csv(complete_filename, rows_to_be_saved):
    ...
    """
    with open(
            complete_filename.encode('utf-8'), 'w', newline='',
            encoding='UTF-8'
    ) as csv_file:
...
def create_directory(basedir, path_to_create):
    ...
    :param basedir: directory that already exists (parent path where new path
    must be included)
    :param path_to_create: directory to be created
    :return:
            - "" if path was correctly created or error message if there was
            an error
...
class ExportExecution:
        ...
        for group in group_list:
            ...
            if hasattr(group, 'experimental_protocol'):
                ...
                for eeg_step in eeg_setting_list:
                    ...
                    eeg_step_name = "STEP_" + eeg_step.numeration + "_" + \
                                    eeg_step.type.upper()
                    path_eeg_directory = \
                        path.join(directory_experimental_protocol, eeg_step_name)
                    if not path.exists(path_eeg_directory):
                        error_msg, path_eeg_directory = create_directory(
                            directory_experimental_protocol, eeg_step_name
                        )
                        if error_msg != "":
                            return error_msg

                    export_eeg_directory = path.join(
                        export_directory_experimental_protocol, eeg_step_name
                    )
                    if hasattr(eeg_step, 'eeg'):
                        default_eeg = get_object_or_404(EEG, pk=eeg_step.id)
                        eeg_default_setting_description = \
                            get_eeg_setting_description(default_eeg.eeg_setting.id)
                        if eeg_default_setting_description:
                            ...
                            complete_filename_eeg_setting = path.join(
                                path_eeg_directory, eeg_setting_filename
                            )

                            self.files_to_zip_list.append(
                                [complete_filename_eeg_setting,
                                 export_eeg_directory]
                            )

                            with open(
                                    complete_filename_eeg_setting.encode('utf-8'),
                                    'w', newline='', encoding='UTF-8'
                            ) as outfile:
                                json.dump(
                                    eeg_default_setting_description, outfile, indent=4
                                )

                    # additional files
                    self.export_experimental_protocol_additional_files(
                        eeg_step, path_eeg_directory, export_eeg_directory
                    )
                ...
                for emg_step in emg_setting_list:
                    ...
                    emg_step_name = \
                        "STEP_" + emg_step.numeration + "_" + \
                        emg_step.type.upper()
                    path_emg_directory = \
                        path.join(directory_experimental_protocol, emg_step_name)
                    if not path.exists(path_emg_directory):
                        error_msg, path_emg_directory = create_directory(
                            directory_experimental_protocol, emg_step_name
                        )
                        if error_msg != "":
                            return error_msg

                    export_emg_directory = path.join(
                        export_directory_experimental_protocol, emg_step_name
                    )

                    if hasattr(emg_step, 'emg'):
                        default_emg = get_object_or_404(EMG, pk=emg_step.id)
                        emg_default_setting_description = \
                            get_emg_setting_description(default_emg.emg_setting.id)
                        if emg_default_setting_description:
                            emg_setting_filename = "%s.json" % "emg_default_setting"
                            complete_filename_emg_setting = \
                                path.join(path_emg_directory, emg_setting_filename)

                            self.files_to_zip_list.append(
                                [complete_filename_emg_setting, export_emg_directory]
                            )

                            with open(
                                    complete_filename_emg_setting.encode('utf-8'),
                                    'w', newline='', encoding='UTF-8'
                            ) as outfile:
                                json.dump(
                                    emg_default_setting_description, outfile,
                                    indent=4
                                )

                    # additional files
                    self.export_experimental_protocol_additional_files(
                        emg_step, path_emg_directory, export_emg_directory
                    )
                ...
                for tms_step in tms_setting_list:
                    ...
                    path_tms_directory = \
                        path.join(directory_experimental_protocol, tms_step_name)
                    if not path.exists(path_tms_directory):
                        error_msg, path_tms_directory = create_directory(
                            directory_experimental_protocol, tms_step_name
                        )
                        if error_msg != "":
                            return error_msg

                    export_tms_directory = path.join(
                        export_directory_experimental_protocol, tms_step_name
                    )

                    if hasattr(tms_step, 'tms'):
                        default_tms = get_object_or_404(TMS, pk=tms_step.id)
                        tms_default_setting_description = \
                            get_tms_setting_description(default_tms.tms_setting.id)
                        if tms_default_setting_description:
                            tms_setting_filename = "%s.json" % "tms_default_setting"
                            complete_filename_tms_setting = path.join(
                                path_tms_directory, tms_setting_filename
                            )

                            self.files_to_zip_list.append(
                                [complete_filename_tms_setting,
                                 export_tms_directory]
                            )

                            with open(
                                    complete_filename_tms_setting.encode('utf-8'),
                                    'w', newline='', encoding='UTF-8'
                            ) as outfile:
                                json.dump(
                                    tms_default_setting_description, outfile,
                                    indent=4
                                )

                    # additional files
                    self.export_experimental_protocol_additional_files(
                        tms_step, path_tms_directory, export_tms_directory
                    )

                goalkeeper_game_list = Step.objects.filter(
                    group=group, type='goalkeeper_game'
                )
                for goalkeeper_game_step in goalkeeper_game_list:
                    default_goalkeeper_game = get_object_or_404(
                        GoalkeeperGame, pk=goalkeeper_game_step.id
                    )
                    context_tree_default_description = \
                        get_context_tree_description(
                    ...
                    if context_tree_default_description:
                        ...
                        goalkeeper_game_step_name = \
                            "STEP_" + goalkeeper_game_step.numeration + "_" + \
                            goalkeeper_game_step.type.upper()

                        path_goalkeeper_game_directory = path.join(
                            directory_experimental_protocol,
                            goalkeeper_game_step_name
                        )
                        if not path.exists(path_goalkeeper_game_directory):
                            error_msg, path_goalkeeper_game_directory = \
                                create_directory(
                                    directory_experimental_protocol,
                                    goalkeeper_game_step_name
                                )
                        ...
                        goalkeeper_game_setting_filename = \
                            "%s.json" % "goalkeeper_game_default_setting"
                        complete_filename_goalkeeper_game_setting = path.join(
                            path_goalkeeper_game_directory,
                            goalkeeper_game_setting_filename
                        )

                        self.files_to_zip_list.append(
                            [complete_filename_goalkeeper_game_setting,
                             export_goalkeeper_game_directory]
                        )

                        with open(
                                complete_filename_goalkeeper_game_setting.encode('utf-8'),
                                'w', newline='', encoding='UTF-8'
                        ) as outfile:
                            json.dump(
                                context_tree_default_description, outfile,
                                indent=4
                            )

                        # if context_tree have a file
                        setting_file = \
                            default_goalkeeper_game.context_tree.setting_file if \
                                default_goalkeeper_game.context_tree.setting_file else ''
                        if setting_file:
                            context_tree_file = setting_file.file if \
                                setting_file.file else ''
                            if context_tree_file:
                                saved_context_tree_filename = \
                                    context_tree_file.name
                                read_filename_context_tree = path.join(
                                    settings.MEDIA_ROOT, saved_context_tree_filename
                                )
                                context_tree_filename = \
                                    saved_context_tree_filename.split('/')[-1]
                                ...
                                self.files_to_zip_list.append(
                                    [complete_context_tree_filename,
                                     export_goalkeeper_game_directory]
                                )

                    # additional files
                    self.export_experimental_protocol_additional_files(
                        goalkeeper_game_step,
                        path_goalkeeper_game_directory,
                        export_goalkeeper_game_directory
                    )

                stimulus_list = Step.objects.filter(
                    group=group, type='stimulus'
                )
                for stimulus_step in stimulus_list:
                    stimulus = get_object_or_404(Stimulus, pk=stimulus_step.id)
                    if stimulus.media_file:
                        stimulus_step_name = \
                            "STEP_" + stimulus_step.numeration + "_" + \
                            stimulus_step.type.upper()
                        path_stimulus_directory = path.join(
                            directory_experimental_protocol, stimulus_step_name
                        )
                        if not path.exists(path_stimulus_directory):
                            error_msg, path_stimulus_directory = \
                                create_directory(
                                    directory_experimental_protocol,
                                    stimulus_step_name
                                )
                        ...
                        export_stimulus_directory = path.join(
                            export_directory_experimental_protocol,
                            stimulus_step_name
                        )

                        stimulus_setting_filename = \
                            stimulus.media_file.name.split('/')[-1]
                        complete_stimulus_filename = path.join(
                            path_stimulus_directory, stimulus_setting_filename
                        )
                        read_stimulus_filename = path.join(
                            settings.MEDIA_ROOT, stimulus.media_file.name
                        )
                        ...
                        self.files_to_zip_list.append(
                            [complete_stimulus_filename,
                             export_stimulus_directory]
                        )

                        # additional files
                        self.export_experimental_protocol_additional_files(
                            stimulus_step, path_stimulus_directory,
                            export_stimulus_directory
                        )

                generic_data_collection_list = Step.objects.filter(
                    group=group, type='generic_data_collection'
                )
                for generic_data_step in generic_data_collection_list:
                    # create directory of generic_data step
                    generic_data_step_name = \
                        "STEP_" + generic_data_step.numeration + "_" + \
                        generic_data_step.type.upper()
                    path_generic_data_directory = path.join(
                        directory_experimental_protocol,
                        generic_data_step_name
                    )
                    if not path.exists(path_generic_data_directory):
                        error_msg, path_generic_data_directory = \
                            create_directory(
                                directory_experimental_protocol,
                                generic_data_step_name
                            )
                    ...
                    # additional files
                    self.export_experimental_protocol_additional_files(
                        generic_data_step, path_generic_data_directory,
    ...
    def export_experimental_protocol_additional_files(self, step_object, directory_step, export_directory_step):
        ...
        directory_additional_files = path.join(
            directory_step, "Additional_files"
        )
        if len(additional_files_list) > 0 and not \
                path.exists(directory_additional_files):
            error_msg, directory_additional_files = create_directory(
                directory_step, "Additional_files"
            )
            if error_msg != "":
                return error_msg

            export_directory_additional_files = path.join(
                export_directory_step, "Additional_files"
            )

        for additional_file in additional_files_list:
            file_name = additional_file.file.name
            complete_additional_filename = path.join(
                directory_additional_files, file_name.split('/')[-1]
            )
            ...
            self.files_to_zip_list.append(
                [complete_additional_filename,
                 export_directory_additional_files]
            )
    ...
    def process_group_inclusion_disease(self, inclusion_criteria_list):
        ...
        if inclusion_criteria_list:
            headers, fields = self.get_headers_and_fields(
                classification_of_disease_fields
            )
            export_rows_classification_of_disease = [headers]
            for inclusion_criteria in inclusion_criteria_list:
                field_list = \
                    [inclusion_criteria.code, inclusion_criteria.description,
    ...
    def include_data_from_group(self, experiment_id):
        ...
        for group in group_list:
            ...
            group_directory = path.join(
                self.get_export_directory(), group_name_directory
            )
            export_group_directory = path.join(
                self.get_input_data("base_directory"), group_name_directory
            )
            self.per_group_data[group_id]['group']['directory'] = group_directory
            self.per_group_data[group_id]['group']['export_directory'] = \
            ...
            participant_directory = path.join(
                self.per_group_data[group_id]['group']['directory'],
                'Participants_data'
            )
            export_participant_directory = path.join(
                self.per_group_data[group_id]['group']['export_directory'],
            ...
            for step_questionnaire in step_list:
                questionnaire_list = QuestionnaireResponse.objects.filter(
                    step_id=step_questionnaire.id
                )
                for questionnaire in questionnaire_list:
                    participant_code = questionnaire.participant.code
                    # questionnaire response fields list
                    questionnaire_response_fields = json.loads(
                        questionnaire.limesurvey_response
                    )
                    questionnaire_response_list = \
                        questionnaire_response_fields['answers']
                    ...
                    questionnaire_response_fields_list.extend(
                        questionnaire_response_list
                    )
                    ...
                    # questionnaire title
                    questionnaire_default_language = get_object_or_404(
                            QuestionnaireDefaultLanguage,
                            questionnaire_id=step_questionnaire.id
                        )
                    questionnaire_title = \
                        "%s_%s" % (str(questionnaire_code), slugify(
            ...
            eeg_participant_list = EEGData.objects.filter(
            ...
            for eeg_participant in eeg_participant_list:
                ...
                directory_step_name = \
                    'STEP_' + \
                    str(eeg_participant.step.numeration) + "_" + \
                ...
                export_participant_code_directory = path.join(
                    export_participant_directory,
                    participant_code_directory_name
                )
                ...
                self.per_group_data[group_id]['data_per_participant'][participant_code]['eeg_data_list'].append({
                    ...
                    'export_directory_step': path.join(
                        export_participant_code_directory, directory_step_name
                    ),
                })

            emg_participant_list = EMGData.objects.filter(
                participant__in=participant_group_list
            )
            for emg_participant in emg_participant_list:
                ...
                directory_step_name = \
                    'STEP_' + \
                    str(emg_participant.step.numeration) + "_" + \
            ...
            tms_participant_list = TMSData.objects.filter(
                participant__in=participant_group_list
            )
            for tms_participant in tms_participant_list:
                ...
                participant_code_directory_name = \
                    'Participant_' + participant_code
                directory_step_name = \
                    'STEP_' + str(tms_participant.step.numeration) + "_" + \
            ...
            for generic_data in generic_data_list:
                ...
                participant_code_directory_name = \
                    'Participant_' + participant_code
                directory_step_name = \
                    'STEP_' + str(generic_data.step.numeration) + "_" + \
            ...
            goalkeeper_data_list = GoalkeeperGameData.objects.filter(
                participant__in=participant_group_list
            )
            for goalkeeper_data in goalkeeper_data_list:
                participant_code = goalkeeper_data.participant.code
                participant_code_directory_name = \
                    'Participant_' + goalkeeper_data.participant.code
                directory_step_name = \
                    'STEP_' + str(goalkeeper_data.step.numeration) + "_" + \
    ...
    def download_data_per_participant(self):
        ...
        for group_id in self.per_group_data:
            if self.per_group_data[group_id]['data_per_participant']:
                ...
                # ex. EXPERIMENT_DOWNLOAD/Group_group.title/Per_participant_data
                error_msg, group_participants_directory = create_directory(
                    group_directory, per_participant_data
                )
                if error_msg != '':
                    return error_msg
                ...
                for participant_code in self.per_group_data[group_id]['data_per_participant']:
                    participant_code_directory_name = "Participant_" + participant_code
                    # ex. EXPERIMENT_DOWNLOAD/Group_group.title/Participants/PXXXXX
                    participant_directory = path.join(
                        group_participants_directory, participant_code_directory_name
                    )
                    if not path.exists(participant_directory):
                        error_msg, participant_directory = create_directory(
                            group_participants_directory,
                            participant_code_directory_name
                        )
                        if error_msg != '':
                            return error_msg
                    ...
                    if 'questionnaire_data' in \
                        ...
                        for questionnaire_data in questionnaire_list:
                            questionnaire_directory = \
                                questionnaire_data['directory_step']
                            ...
                            if not path.exists(questionnaire_directory):
                                error_msg, questionnaire_directory = \
                                    create_directory(
                                        participant_directory,
                                        questionnaire_data['directory_step_name']
                                    )
                                if error_msg != '':
                                    return error_msg
                            ...
                            # ex. EXPERIMENT_DOWNLOAD/Group_group.title/Participants/PXXXX/Step_XX_Questionnaire
                            export_questionnaire_directory = \
                                questionnaire_data['export_directory_step']
                            filename_questionnaire = \
                                questionnaire_data['questionnaire_filename']
                            complete_filename_questionnaire = path.join(
                                questionnaire_directory, filename_questionnaire
                            )

                            questionnaire_description_fields = \
                                questionnaire_data['questionnaire_response_list']

                            save_to_csv(
                                complete_filename_questionnaire,
                                questionnaire_description_fields
                            )
                    ...
                    if 'eeg_data_list' in self.per_group_data[group_id]['data_per_participant'][participant_code]:
                        ...
                        for eeg_data in eeg_data_list:
                            ...
                            if not path.exists(eeg_directory):
                                error_msg, eeg_directory = create_directory(
                                    participant_directory,
                            ...
                            directory_data_name = \
                                eeg_data['eeg_data_directory_name']
                            path_per_eeg_data = path.join(
                                eeg_directory, directory_data_name
                            )
                            if not path.exists(path_per_eeg_data):
                                # NES_EXPORT/Experiment_data/Group_XXX/Per_participant/Participant_123/Step_X_aaa/EEGData_index
                                error_msg, path_per_eeg_data = create_directory(
                                    eeg_directory, directory_data_name
                                )
                                if error_msg != '':
                                    return error_msg
                            # ex. /NES_EXPORT/Experiment_data/Group_XXX/Per_participant/Participant_123/Step_X_aaa/EEGData_index
                            export_eeg_data_directory = path.join(
                                export_eeg_directory, directory_data_name
                            )

                            eeg_data = get_object_or_404(
                                EEGData, pk=eeg_data['data_id']
                            )
                            ...
                            for file in eeg_data.files.all():
                                ...
                                # ex. EXPERIMENT_DOWNLOAD/Group_group.title/Participants/PXXXX/Step_XX_EEG/EEGData_index/eeg.raw
                                complete_eeg_data_filename = path.join(
                                    path_per_eeg_data, eeg_data_filename
                                )
                                eeg_raw_data_file = path.join(
                                    path.join(settings.BASE_DIR, "media/"),
                                    file.file.name
                                )
                                ...
                                eeg_setting_description = \
                                    get_eeg_setting_description(eeg_data.eeg_setting_id)
                                if eeg_setting_description:
                                    eeg_setting_filename = \
                                        '%s_%s.json' % \
                                        (eeg_data_filename.split(".")[0],
                                         "setting_description")

                                    # ex. EXPERIMENT_DOWNLOAD/Group_group.title/Participants/PXXXX/Step_XX_EEG/eeg_rawfilename_setting_description.json
                                    complete_setting_filename = path.join(
                                        path_per_eeg_data, eeg_setting_filename
                                    )
                                    ...
                                    with open(
                                            complete_setting_filename.encode('utf-8'),
                                            'w', newline='', encoding='UTF-8'
                                    ) as outfile:
                                        json.dump(
                                            eeg_setting_description, outfile,
                                            indent=4
                                        )
                            ...
                            if hasattr(
                                ...
                                eeg_electrode_localization_system = \
                                    eeg_data.eeg_setting.eeg_electrode_localization_system
                                if hasattr(
                                        eeg_electrode_localization_system,
                                        'map_image_file'
                                ):
                                    sensor_position_filename = \
                                        '%s.png' % 'sensor_position'
                                    map_filename = \
                                        eeg_electrode_localization_system.map_image_file.name
                                    sensors_positions_image = \
                                        settings.BASE_DIR + \
                                        settings.MEDIA_URL + map_filename
                                    ...
                                    self.files_to_zip_list.append(
                                        [complete_sensor_position_filename,
                                         export_eeg_data_directory]
                                    )
                    ...
                    if 'emg_data_list' in self.per_group_data[group_id]['data_per_participant'][participant_code]:
                        ...
                        for emg_data in emg_data_list:
                            ...
                            if not path.exists(emg_directory):
                                error_msg, emg_directory = create_directory(
                                    participant_directory,
                            ...
                            export_emg_directory = \
                                emg_data['export_directory_step']

                            # to create EMGData directory
                            directory_data_name = \
                                emg_data['emg_data_directory_name']
                            path_per_emg_data = path.join(
                                emg_directory, directory_data_name
                            )
                            if not path.exists(path_per_emg_data):
                                # ex. NES_EXPORT/Experiment_data/Group_XXX/Per_participant/Participant_123/Step_X_aaa/EMGData_index
                                ...
                                if error_msg != '':
                                    return error_msg
                            # ex. /NES_EXPORT/Experiment_data/Group_XXX/Per_participant/Participant_123/Step_X_aaa/EMGData_index
                            export_emg_data_directory = path.join(
                                export_emg_directory, directory_data_name
                            )

                            emg_data = get_object_or_404(
                                EMGData, pk=emg_data['data_id']
                            )
                            ...
                            for file in emg_data.files.all():
                                ...
                                # ex. EXPERIMENT_DOWNLOAD/Group_group.title/Participants/PXXXX/Step_XX_EMG/EMGData_index/emg.raw
                                complete_emg_data_filename = path.join(
                                    path_per_emg_data, emg_data_filename
                                )
                                emg_raw_data_file = path.join(
                                    settings.BASE_DIR, 'media', file.file.name
                                )
                                ...
                                emg_setting_description = \
                                    get_emg_setting_description(emg_data.emg_setting_id)
                                if emg_setting_description:
                                    emg_setting_filename = \
                                        '%s_%s.json' % \
                                        (emg_data_filename.split(".")[0],
                                         "setting_description")

                                    # ex. EXPERIMENT_DOWNLOAD/Group_group.title/Participants/PXXXX/Step_XX_EMG
                                    # /EMGData_index/emg_rawfilename_setting_description.json
                                    complete_setting_filename = path.join(
                                        path_per_emg_data, emg_setting_filename
                                    )
                                    ...
                                    with open(
                                        ...
                                        json.dump(
                                            emg_setting_description, outfile,
                                            indent=4
                                        )
                    ...
                    if 'tms_data' in \
                        ...
                        for tms_data in tms_data_list:
                            ...
                            if not path.exists(tms_directory):
                                error_msg, tms_directory = create_directory(
                                    participant_directory,
                            ...
                            # ex. EXPERIMENT_DOWNLOAD/Group_group.title/Participants/PXXXX/Step_XX_TMS
                            export_tms_directory = \
                                tms_data['export_directory_step']

                            tms_data_description = get_tms_data_description(
                                tms_data['data_id']
                            )
                            if tms_data_description:
                                tms_data_filename = \
                                    '%s.json' % 'tms_data_description'
                                # ex. EXPERIMENT_DOWNLOAD/Group_group.title/Participants/PXXXX/Step_XX_TMS/tms_data_description.json
                                complete_data_filename = path.join(
                                    tms_directory, tms_data_filename
                                )
                                ...
                                with open(
                                        complete_data_filename.encode('utf-8'),
                                        'w', newline='', encoding='UTF-8'
                                ) as outfile:
                                    json.dump(
                                        tms_data_description, outfile,
                                        indent=4
                                    )

                            # TMS hotspot position image file
                            tms_data = get_object_or_404(
                                TMSData, pk=tms_data['data_id']
                            )
                            ...
                            if tms_data.localization_system_image:
                                ...
                                if hotspot_image:
                                    hotspot_map_filename = \
                                        hotspot_image.split("/")[-1]
                                    # ex. EXPERIMENT_DOWNLOAD/Group_group.title/Participants/PXXXX/Step_XX_TMS/hotspot_image.png
                                    complete_hotspot_filename = path.join(
                                        tms_directory, hotspot_map_filename
                                    )
                    ...
                    if 'additional_data_list' in \
                        ...
                        for additional_data in additional_data_list:
                            ...
                            if not path.exists(additional_data_directory):
                                error_msg, additional_data_directory = \
                                    create_directory(
                                    participant_directory,
                                        additional_data['directory_step_name']
                                    )
                                if error_msg != '':
                                    return error_msg
                            # ex. EXPERIMENT_DOWNLOAD/Group_group.title/Participants/PXXXX/Step_XX_step_TYPE
                            export_data_directory = \
                                additional_data['export_directory_step']

                            # to create AdditionalData directory
                            directory_data_name = \
                                additional_data['additional_data_directory_name']
                            path_per_additional_data = path.join(
                                additional_data_directory, directory_data_name
                            )
                            if not path.exists(path_per_additional_data):
                                # ex. NES_EXPORT/Experiment_data/Group_XXX/Per_participant/Participant_123/Step_X_aaa/AdditionalData_index
                                error_msg, path_per_additional_data = \
                                    create_directory(
                                        additional_data_directory,
                                        directory_data_name
                                    )
                            ...
                            export_additional_data_directory = path.join(
                                export_data_directory, directory_data_name
                            )

                            data_file = get_object_or_404(
                                AdditionalData, pk=additional_data['data_id']
                            )

                            for file in data_file.files.all():
                                file_name = file.file.name.split('/')[-1]
                                # read file from repository
                                additional_data_filename = path.join(
                                    settings.BASE_DIR, 'media'
                                ) + '/' + file.file.name

                                # ex. NES_EXPORT/Experiment_data/Group_XXX/Participants/PXXXX/Step_XX_step_TYPE/file_name.format_type
                                complete_additional_data_filename = path.join(
                                    path_per_additional_data, file_name
                                )
                                with open(additional_data_filename, 'rb') as f:
                                    data = f.read()

                                with open(
                                        complete_additional_data_filename, 'wb'
                                ) as f:
                                    f.write(data)

                                self.files_to_zip_list.append(
                                    [complete_additional_data_filename,
                                     export_additional_data_directory]
                                )

                    if 'goalkeeper_data_list' in \
                            self.per_group_data[group_id]['data_per_participant'][participant_code]:
                        goalkeeper_data_list = \
                            self.per_group_data[group_id]['data_per_participant'][participant_code]['goalkeeper_data_list']

                        for goalkeeper_data in goalkeeper_data_list:
                            goalkeeper_game_directory = \
                                goalkeeper_data['directory_step_name']
                            if not path.exists(goalkeeper_game_directory):
                                # ex. EXPERIMENT_DOWNLOAD/Group_group.title/Participants/PXXXX/Step_XX_step_TYPE
                                error_msg, goalkeeper_step_directory = \
                                    create_directory(
                                        participant_directory,
                                        goalkeeper_data['directory_step_name']
                                    )
                                ...
                                # ex. EXPERIMENT_DOWNLOAD/Group_group.title/Participants/PXXXX/Step_XX_step_TYPE
                                export_data_directory = \
                                    goalkeeper_data['export_directory_step']

                                # to create GoalkeeperData directory
                                directory_data_name = \
                                    goalkeeper_data['goalkeeper_data_directory_name']
                                path_per_goalkeeper_data = path.join(
                                    goalkeeper_step_directory,
                                    directory_data_name
                                )
                                if not path.exists(path_per_goalkeeper_data):
                                    # NES_EXPORT/Experiment_data/Group_XXX/Per_participant/Participant_123/Step_X_aaa/GoalkeeperData_index
                                    error_msg, path_per_goalkeeper_data = \
                                        create_directory(
                                            goalkeeper_step_directory,
                                            directory_data_name
                                        )
                                    if error_msg != '':
                                        return error_msg
                                # ex. NES_EXPORT/Experiment_data/Group_XXX/Per_participant/Participant_123/Step_X_aaa/GoalkeeperData_index
                    ...
                    if 'generic_data_list' in \
                        ...
                        for generic_data in generic_data_list:
                            generic_data_directory = \
                                generic_data['directory_step_name']
                            if not path.exists(generic_data_directory):
                                # ex. EXPERIMENT_DOWNLOAD/Group_group.title/Participants/PXXXX/Step_XX_step_TYPE
                                error_msg, generic_step_directory = \
                                    create_directory(
                                        participant_directory,
                                        generic_data['directory_step_name']
                                    )
                                ...
                                # ex. EXPERIMENT_DOWNLOAD/Group_group.title/Participants/PXXXX/Step_XX_step_TYPE
                                export_data_directory = \
                                    generic_data['export_directory_step']

                                # to create GoalkeeperData directory
                                directory_data_name = \
                                    generic_data['generic_data_directory_name']
                                path_per_generic_data = path.join(
                                    generic_step_directory, directory_data_name
                                )
                                if not path.exists(path_per_generic_data):
                                    # ex. NES_EXPORT/Experiment_data/Group_XXX/Per_participant/Participant_123/Step_X_aaa/GenericData_index
                                    error_msg, path_per_generic_data = \
                                        create_directory(
                                            generic_step_directory,
                                            directory_data_name
                                        )
                                    if error_msg != "":
                                        return error_msg
                                ...
                                data_file = get_object_or_404(
                                    ...
                                    pk=generic_data['data_id']
                                )

                                for generic_file in data_file.files.all():
                                    generic_file_name = \
                                        generic_file.file.name.split('/')[-1]
                                    # read file from repository
                                    path_generic_file = path.join(
                                        settings.BASE_DIR, 'media'
                                    ) + '/' + generic_file.file.name

                                    # ex. NES_EXPORT/Experiment_data/Group_XXX/Participants/PXXXX/
                                    # Step_XX_step_TYPE/GenericData_index/file_name.format_type
                                    complete_generic_filename = path.join(
                                        path_per_generic_data,
                                        generic_file_name
                                    )
    ...
    def download_data_per_questionnaire(self):
        ...
        per_questionnaire_metadata = \
            self.input_data['questionnaire_metadata_directory']
        for group_id in self.per_group_data:
            ...
            if 'questionnaire_data' in self.per_group_data[group_id]:
                ...
                error_msg, group_questionnaire_directory = create_directory(
                    group_directory, per_questionnaire_data
                )
                if error_msg != '':
                    return error_msg
                export_group_questionnaire_directory = path.join(
                    export_directory_group, per_questionnaire_data
                )

                # create 'questionnaire_metadata' directory
                questionnaire_metadata_directory = path.join(
                    group_directory, per_questionnaire_metadata
                )
                if not path.exists(questionnaire_metadata_directory):
                    error_msg, questionnaire_metadata_directory = \
                        create_directory(
                            group_directory, per_questionnaire_metadata
                        )
                    if error_msg != '':
                ...
                for questionnaire_code in questionnaire_list:
                    questionnaire_description_fields = \
                        questionnaire_list[questionnaire_code]['response_list']
                    questionnaire_description_fields.insert(
                        0, questionnaire_list[questionnaire_code]['header']
                    )
                    questionnaire_title = \
                        questionnaire_list[questionnaire_code]['questionnaire_title']
                    # create questionnaire_title directory
                    error_msg, questionnaire_directory = create_directory(
                        group_questionnaire_directory,
                        questionnaire_title
                    )
                    if error_msg != '':
                        return error_msg
                    export_questionnaire_directory = path.join(
                        export_group_questionnaire_directory,
                        questionnaire_title
                    )

                    # create directory metadata by each questionnaire
                    error_msg, questionnaire_metadata_directory_name = \
                        create_directory(
                    ...
                    questionnaire_filename = \
                        questionnaire_list[questionnaire_code]['questionnaire_filename']
                    complete_filename_questionnaire = path.join(
                        questionnaire_directory, questionnaire_filename
                    )
                    ...
                    if 'questionnaire_metadata' in self.per_group_data[group_id]:
                        ...
                        for questionnaire_language in questionnaire_metadata_list:
                            questionnaire_metadata_fields = \
                                questionnaire_metadata_list[questionnaire_language]['metadata_fields']
                            filename_questionnaire_metadata = \
                                questionnaire_metadata_list[questionnaire_language]['filename']
                            complete_filename_questionnaire_metadata = \
                                path.join(
                                    questionnaire_metadata_directory_name,
                                    filename_questionnaire_metadata
                                )

                            with open(
                                    complete_filename_questionnaire_metadata, 'w'
                            ) as f:
...
def get_eeg_setting_description(eeg_setting_id):
    ...
    if hasattr(eeg_setting, 'eeg_amplifier_setting'):
        eeg_amplifier_setting_attributes = \
            vars(eeg_setting.eeg_amplifier_setting)
        eeg_amplifier_setting_attributes = handling_values(
            eeg_amplifier_setting_attributes
        )
        eeg_amplifier_attributes = vars(
            eeg_setting.eeg_amplifier_setting.eeg_amplifier
        )
        eeg_amplifier_attributes = handling_values(
            eeg_amplifier_attributes
        )
        impedance_description = ''
        if eeg_amplifier_attributes['input_impedance'] and \
                eeg_amplifier_attributes['input_impedance_unit']:
            impedance_description = \
                eeg_amplifier_attributes['input_impedance'] + ' (' + \
                eeg_amplifier_attributes['input_impedance_unit'] + ')'
        description['eeg_amplifier_setting'] = {
            ...
            'sampling_rate_setted':
                eeg_amplifier_setting_attributes['sampling_rate'],
            'number_of_channels_used':
                eeg_amplifier_setting_attributes['number_of_channels_used'],
            'gain (equipment)': eeg_amplifier_attributes['gain'],
            'number_of_channels (equipment)':
                eeg_amplifier_attributes['number_of_channels'],
            'common_mode_rejection_ratio':
                eeg_amplifier_attributes['common_mode_rejection_ratio'],
            'input_impedance': impedance_description,
            'amplifier_detection_type_name':
                eeg_amplifier_attributes['amplifier_detection_type_name'],
    ...
    if hasattr(eeg_setting, 'eeg_filter_setting'):
        ...
        eeg_filter_setting_attributes = handling_values(
            eeg_filter_setting_attributes
        )
        description['eeg_filter_setting'] = {
            'filter_type': eeg_filter_setting_attributes['eeg_filter_type_name'],
            'description':
                eeg_filter_setting_attributes['eeg_filter_type_description'],
    ...
    if hasattr(eeg_setting, 'eeg_electrode_localization_system'):
        eeg_electrode_localization_system_attributes = vars(
            eeg_setting.eeg_electrode_localization_system
        )
        eeg_electrode_localization_system_attributes = handling_values(
            eeg_electrode_localization_system_attributes
        )
        description['eeg_electrode_localization_system'] = {
            'name': eeg_electrode_localization_system_attributes['name'],
            'description':
                eeg_electrode_localization_system_attributes['description'],
            'image_filename':
                eeg_electrode_localization_system_attributes['map_image_file'].split('/')[-1],
        ...
        for eeg_electrode_position in eeg_electrode_position_list:
            ...
            eeg_electrode_position_attributes = handling_values(
                eeg_electrode_position_attributes
            )
            description['eeg_electrode_localization_system']['electrode_position_list'].append({
                'name': eeg_electrode_position_attributes['name'],
                'coordinate_x':
                    eeg_electrode_position_attributes['coordinate_x'],
                'coordinate_y':
                    eeg_electrode_position_attributes['coordinate_y'],
                'channel_index':
                    eeg_electrode_position_attributes['channel_index'],
                'electrode_model': {}
            })

            if hasattr(eeg_electrode_position, 'electrode_model'):
                electrode_model_attributes = vars(
                    eeg_electrode_position.electrode_model
                )
                electrode_model_attributes = handling_values(
                    electrode_model_attributes
                )
                impedance_description = ''
                electrode_distance_description = ''
                if electrode_model_attributes['impedance'] and \
                        electrode_model_attributes['impedance_unit']:
                    impedance_description = \
                        electrode_model_attributes['impedance'] + ' (' + \
                        electrode_model_attributes['impedance_unit'] + ')'
                if electrode_model_attributes['inter_electrode_distance'] and \
                        electrode_model_attributes['inter_electrode_distance_unit']:
                    electrode_distance_description = \
                        electrode_model_attributes['inter_electrode_distance'] + \
                        ' (' + \
                        electrode_model_attributes['inter_electrode_distance_unit'] + \
                        ')'
                description['eeg_electrode_localization_system']['electrode_position_list'][-1]['electrode_model']['model_name'] = \
                    electrode_model_attributes['name']
                description['eeg_electrode_localization_system']['electrode_position_list'][-1]['electrode_model']['electrode type'] = \
                    electrode_model_attributes['electrode_type']
                description['eeg_electrode_localization_system']['electrode_position_list'][-1]['electrode_model']['description'] = \
                    electrode_model_attributes['description']
                description['eeg_electrode_localization_system']['electrode_position_list'][-1]['electrode_model']['material'] = \
                    electrode_model_attributes['material']
                description['eeg_electrode_localization_system']['electrode_position_list'][-1]['electrode_model']['usability'] = \
                    electrode_model_attributes['usability']
                description['eeg_electrode_localization_system']['electrode_position_list'][-1]['electrode_model']['impedance'] = \
                    impedance_description
                description['eeg_electrode_localization_system']['electrode_position_list'][-1]['electrode_model']['distance_inter_electrode'] = \
                    electrode_distance_description
                description['eeg_electrode_localization_system']['electrode_position_list'][-1]['electrode_model']['electrode_configuration_name'] = \
                    electrode_model_attributes['electrode_configuration_name']

                if hasattr(eeg_electrode_position.electrode_model, 'surfaceelectrode'):
                    surface_electrode_attributes = vars(
                        eeg_electrode_position.electrode_model.surfaceelectrode
                    )
                    surface_electrode_attributes = handling_values(
                        surface_electrode_attributes
                    )
                    electrode_shape_measure_description = ''
                    if surface_electrode_attributes['electrode_shape_measure_value'] and \
                            surface_electrode_attributes['electrode_shape_measure_unit']:
                        electrode_shape_measure_description = \
                            surface_electrode_attributes['electrode_shape_measure'] + \
                            ' (' + surface_electrode_attributes['electrode_shape_measure_unit'] + \
                            ')'

                    description['eeg_electrode_localization_system']['electrode_position_list'][-1]['electrode_model']['surface_electrode'] = {}
                    description['eeg_electrode_localization_system']['electrode_position_list'][-1]['electrode_model']['surface_electrode']['conduction_type'] = \
                        surface_electrode_attributes['conduction_type']
                    description['eeg_electrode_localization_system']['electrode_position_list'][-1]['electrode_model']['surface_electrode']['electrode_mode'] = \
                        surface_electrode_attributes['electrode_mode']
                    description['eeg_electrode_localization_system']['electrode_position_list'][-1]['electrode_model']['surface_electrode']['electrode_shape_measure'] = \
                        electrode_shape_measure_description
                    description['eeg_electrode_localization_system']['electrode_position_list'][-1]['electrode_model']['surface_electrode']['electrode_shape_name'] = \
                        surface_electrode_attributes['electrode_shape_name']
                ...
                if hasattr(eeg_electrode_position.electrode_model, 'intramuscular_electrode'):
                    ...
                    intramuscular_electrode_attributes = handling_values(
                        intramuscular_electrode_attributes
                    )
                    description['eeg_electrode_localization_system']['electrode_position_list'][-1]['electrode_model']['intramuscular_electrode'] = {}
                    description['eeg_electrode_localization_system']['electrode_position_list'][-1]['electrode_model']['intramuscular_electrode']['strand'] = \
                        intramuscular_electrode_attributes['strand']
                    description['eeg_electrode_localization_system']['electrode_position_list'][-1]['electrode_model']['intramuscular_electrode']['insulation_material_name'] = \
                        intramuscular_electrode_attributes['insulation_material_name']
                    description['eeg_electrode_localization_system']['electrode_position_list'][-1]['electrode_model']['intramuscular_electrode']['insulation_material_description'] = \
                        intramuscular_electrode_attributes['insulation_material_description']
                    description['eeg_electrode_localization_system']['electrode_position_list'][-1]['electrode_model']['intramuscular_electrode']['lenght_of_exposed_tip'] = \
                ...
                if hasattr(eeg_electrode_position.electrode_model, 'needle_electrode'):
                    needle_electrode_attributes = vars(
                        eeg_electrode_position.electrode_model.needle_electrode
                    )
                    needle_electrode_attributes = handling_values(
                        needle_electrode_attributes
                    )
                    size_description = ''
                    if needle_electrode_attributes['size_of_conductive_contact_points_at_the_tip'] and \
                            needle_electrode_attributes['size_unit']:
                        size_description = \
                            surface_electrode_attributes['size_of_conductive_contact_points_at_the_tip'] \
                            + ' (' + surface_electrode_attributes['size_unit'] + ')'
                    description['eeg_electrode_localization_system']['electrode_position_list'][-1]['electrode_model']['needle_electrode'] = {}
                    description['eeg_electrode_localization_system']['electrode_position_list'][-1]['electrode_model']['needle_electrode']['size'] = \
                        needle_electrode_attributes['size']
                    description['eeg_electrode_localization_system']['electrode_position_list'][-1]['electrode_model']['needle_electrode']['size_of_conductive_contact_points_at_the_tip'] = \
                        size_description
                    description['eeg_electrode_localization_system']['electrode_position_list'][-1]['electrode_model']['needle_electrode']['number_of_conductive_contact_points_at_the_tip'] = \
...
def get_emg_setting_description(emg_setting_id):
    ...
    description = {
        ...
        'acquisition_software':
            emg_setting_attributes['acquisition_software_version'],
    }

    if hasattr(emg_setting, 'emg_ad_converter_setting'):
        emg_ad_converter_setting_attributes = vars(
            emg_setting.emg_ad_converter_setting
        )
        emg_ad_converter_setting_attributes = handling_values(
            emg_ad_converter_setting_attributes
        )

        description['emg_ad_converter_setting'] = {
            'sampling_rate_setted':
                emg_ad_converter_setting_attributes['sampling_rate']
        }

        ad_converter_attributes = vars(
            emg_setting.emg_ad_converter_setting.ad_converter
        )
        ad_converter_attributes = handling_values(ad_converter_attributes)
        description['emg_ad_converter_setting']['ad_converter'] = \
            ad_converter_attributes['identification']
        description['emg_ad_converter_setting']['sample_rate (equipment)'] = \
            ad_converter_attributes['sampling_rate']
        description['emg_ad_converter_setting']['signal_to_noise (equipment)'] = \
            ad_converter_attributes['signal_to_noise_rate']
        description['emg_ad_converter_setting']['resolution (equipment)'] = \
            ad_converter_attributes['resolution']

    if hasattr(emg_setting, 'emg_digital_filter_setting'):
        emg_digital_filter_setting = vars(
            emg_setting.emg_digital_filter_setting
        )
        emg_digital_filter_setting = handling_values(
    ...
    # to_many
    emg_electrode_setting_list = EMGElectrodeSetting.objects.filter(
    ...
    for emg_electrode_setting in emg_electrode_setting_list:
        ...
        electrode_model_attributes = vars(
            emg_electrode_setting.electrode_model
        )
        electrode_model_attributes = handling_values(
            electrode_model_attributes
        )
        ...
        if hasattr(emg_electrode_setting, 'emg_amplifier_setting'):
            emg_amplifier_setting_attributes = vars(
                emg_electrode_setting.emg_amplifier_setting
            )
            emg_amplifier_setting_attributes = handling_values(
                emg_amplifier_setting_attributes
            )
            emg_amplifier_attributes = vars(
                emg_electrode_setting.emg_amplifier_setting.amplifier
            )
            emg_amplifier_attributes = handling_values(
                emg_amplifier_attributes
            )
            impedance_description = \
                emg_amplifier_attributes['input_impedance'] + ' (' + \
                emg_amplifier_attributes['input_impedance_unit'] + ')'
            emg_electrode_setting_dict['emg_amplifier_setting'] = {
                'identification': emg_amplifier_attributes['identification'],
                'manufacturer_name':
                    emg_amplifier_attributes['manufacturer_name'],
                ...
                'number_of_channels (equipment)':
                    emg_amplifier_attributes['number_of_channels'],
                'common_mode_rejection_ratio':
                    emg_amplifier_attributes['common_mode_rejection_ratio'],
                'input_impedance': impedance_description,
                'amplifier_detection_type_name':
                    emg_amplifier_attributes['amplifier_detection_type_name'],
                'tethering_system_name':
                    emg_amplifier_attributes['tethering_system_name'],
            }
            ...
            if hasattr(
                ...
                emg_amplifier_setting = \
                    emg_electrode_setting.emg_amplifier_setting
                emg_analog_filter_setting_attributes = vars(
                    emg_amplifier_setting.emg_analog_filter_setting
                )
                emg_analog_filter_setting_attributes = handling_values(
                    emg_analog_filter_setting_attributes
                )
                ...
                emg_electrode_setting_dict['emg_amplifier_setting']['emg_analog_filter_setting'] = {
                    ...
                    'high_pass':
                        emg_analog_filter_setting_attributes['high_pass'],
                    'low_band_pass':
                        emg_analog_filter_setting_attributes['low_band_pass'],
                    'high_band_pass':
                        emg_analog_filter_setting_attributes['high_band_pass'],
                    'low_notch':
                        emg_analog_filter_setting_attributes['low_notch'],
                    'high_notch':
                        emg_analog_filter_setting_attributes['high_notch'],
                    'order': emg_analog_filter_setting_attributes['order'],
                }

        if hasattr(emg_electrode_setting, 'emg_preamplifier_setting'):
            emg_preamplifier_setting_attributes = vars(
                emg_electrode_setting.emg_preamplifier_setting
            )
            emg_preamplifier_setting_attributes = handling_values(
                emg_preamplifier_setting_attributes
            )
            amplifier_attributes = vars(
            ...
            if amplifier_attributes['input_impedance'] and \
                    amplifier_attributes['input_impedance_unit']:
                preamplifier_impedance_description = \
                    amplifier_attributes['input_impedance'] + ' (' + \
                    amplifier_attributes['input_impedance_unit'] + ')'
            ...
            emg_electrode_setting_dict['emg_preamplifier_setting'] = {
                ...
                'common_mode_rejection_ratio':
                    amplifier_attributes['common_mode_rejection_ratio'],
                'impedance': preamplifier_impedance_description,
                'detection type':
                    amplifier_attributes['amplifier_detection_type_name'],
                'tethering system':
                    amplifier_attributes['tethering_system_name'],
            }
            ...
            if hasattr(
                ...
                emg_preamplifier_filter_setting_attributes = handling_values(
                    emg_preamplifier_filter_setting_attributes
                )

                emg_electrode_setting_dict['emg_preamplifier_setting']['emg_preamplifier_filter_setting'] = {
                    'low_pass':
                        emg_preamplifier_filter_setting_attributes['low_pass'],
                    'high_pass':
                        emg_preamplifier_filter_setting_attributes['high_pass'],
                    'low_band_pass':
                        emg_preamplifier_filter_setting_attributes['low_band_pass'],
                    'high_band_pass':
                        emg_preamplifier_filter_setting_attributes['high_band_pass'],
                    'low_notch':
                        emg_preamplifier_filter_setting_attributes['low_notch'],
                    'high_notch':
                        emg_preamplifier_filter_setting_attributes['high_notch'],
                    'order':
                        emg_preamplifier_filter_setting_attributes['order'],
                }
        ...
        if hasattr(emg_electrode_setting, 'emg_electrode_placement_setting'):
            ...
            emg_electrode_placement_setting_attributes = handling_values(
                emg_electrode_placement_setting_attributes
            )
            emg_electrode_setting_dict['emg_electrode_placement_setting'] = {
                'muscle name':
                    emg_electrode_placement_setting_attributes['muscle_name'],
                'muscle side':
                    emg_electrode_placement_setting_attributes['muscle_side'],
                'remarks':
                    emg_electrode_placement_setting_attributes['remarks'],
            }
            ...
            if hasattr(
                ...
                emg_electrode_placement_setting = \
                    emg_electrode_setting.emg_electrode_placement_setting
                emg_electrode_placement_attributes = vars(
                    emg_electrode_placement_setting.emg_electrode_placement
                )
                emg_electrode_setting_dict['emg_electrode_placement_setting']['electrode_placement'] = {
                    'standardization system':
                        emg_electrode_placement_attributes['standardization_system_name'],
                    'standardization system description':
                        emg_electrode_placement_attributes['standardization_system_description'],
                    'muscle_anatomy_origin':
                        emg_electrode_placement_attributes['muscle_anatomy_origin'],
                    'muscle_anatomy_insertion':
                        emg_electrode_placement_attributes['muscle_anatomy_insertion'],
                    'muscle_anatomy_function':
                        emg_electrode_placement_attributes['muscle_anatomy_function'],
                    'location': emg_electrode_placement_attributes['location'],
                    'placement type':
                        emg_electrode_placement_attributes['placement_type'],
                }
                ...
                if emg_electrode_placement.placement_type == 'intramuscular':
                    ...
                    emg_intramuscular_placement_attributes = handling_values(
                        emg_intramuscular_placement_attributes
                    )
                    emg_electrode_setting_dict['emg_electrode_placement_setting']['electrode_placement']['placement_type_description'] = {
                            'method_of_insertion':
                                emg_intramuscular_placement_attributes['method_of_insertion'],
                            'depth_of_insertion':
                                emg_intramuscular_placement_attributes['depth_of_insertion'],
                        }

                if emg_electrode_placement.placement_type == 'needle':
                    emg_needle_placement = get_object_or_404(
                        EMGNeedlePlacement, pk=emg_electrode_placement.id
                    )
                    emg_needle_placement_attributes = vars(
                        emg_needle_placement
                    )
                    emg_needle_placement_attributes = handling_values(
                        emg_needle_placement_attributes
                    )
                    emg_electrode_setting_dict['emg_electrode_placement_setting']['electrode_placement'][
                        'placement_type_description'] = {
                            'depth_of_insertion':
                                emg_needle_placement_attributes['depth_of_insertion'],
                        }

                if emg_electrode_placement.placement_type == 'surface':
                    emg_surface_placement = get_object_or_404(
                        EMGSurfacePlacement, pk=emg_electrode_placement.id
                    )
                    emg_surface_placement_attributes = vars(
                        emg_surface_placement
                    )
                    emg_surface_placement_attributes = handling_values(
                        emg_surface_placement_attributes
                    )
                    emg_electrode_setting_dict['emg_electrode_placement_setting']['electrode_placement']['placement_type_description'] = {
                            'start_posture':
                                emg_surface_placement_attributes['start_posture'],
                            'orientation':
                                emg_surface_placement_attributes['orientation'],
                            'fixation_on_the_skin':
                                emg_surface_placement_attributes['fixation_on_the_skin'],
                            'reference_electrode':
                                emg_surface_placement_attributes['reference_electrode'],
                            'clinical_test':
                                emg_surface_placement_attributes['clinical_test'],
                        }
...
def get_tms_data_description(tms_data_id):
    ...
    tms_description['stimulation_description'] = {
        ...
        'resting_motor threshold-RMT(%)':
            tms_data_attributes['resting_motor_threshold'],
        ...
        'interval_between_pulses':
            tms_data_attributes['interval_between_pulses'],
        'interval_between_pulses_unit':
            tms_data_attributes['interval_between_pulses_unit'],
        'repetitive_pulse_frequency':
            tms_data_attributes['repetitive_pulse_frequency'],
        'coil_orientation':
            tms_data_attributes['coil_orientation'],
        'coil_orientation_angle':
            tms_data_attributes['coil_orientation_angle'],
        'second_test_pulse_intensity (% over the %RMT)':
            tms_data_attributes['second_test_pulse_intensity'],
        'time_between_mep_trials':
            tms_data_attributes['time_between_mep_trials'],
        'time_between_mep_trials_unit':
            tms_data_attributes['time_between_mep_trials_unit'],
    }

    tms_description['hotspot_position'] = {
        'hot_spot_map_filename':
            tms_data_attributes['hot_spot_map'].split('/')[-1],
        'tms_localization_system_name':
            tms_data_attributes['localization_system_name'],
        'tms_localization_system_description':
            tms_data_attributes['localization_system_description'],
        'brain_area': tms_data_attributes['brain_area_name'],
        'brain_area_description':
            tms_data_attributes['brain_area_description'],
        'brain_area_system_name':
            tms_data_attributes['brain_area_system_name'],
        'brain_area_system_description':
            tms_data_attributes['brain_area_system_description']
    }
    ...
    if hasattr(tms_data.tms_setting, 'tms_device_setting'):
        tms_device_setting_attributes = vars(
            tms_data.tms_setting.tms_device_setting)
        tms_device_setting_attributes = handling_values(
            tms_device_setting_attributes
        )

        tms_coil_model_attributes = vars(
            tms_data.tms_setting.tms_device_setting.coil_model
        )
        tms_coil_model_attributes = handling_values(
            tms_coil_model_attributes
        )

        tms_device_attributes = vars(
            tms_data.tms_setting.tms_device_setting.tms_device
        )
        tms_device_attributes = handling_values(tms_device_attributes)

        tms_description['tms_device_setting'] = {
            'pulse_stimulus_type':
                tms_device_setting_attributes['pulse_stimulus_type'],
            ...
            'material_description':
                tms_coil_model_attributes['material_description'],
...
def get_tms_setting_description(tms_setting_id):
    ...
    if tms_setting:
        tms_setting_description = {
            ...
            'description':
                tms_setting.description if tms_setting.description else '',
        }
...
def get_context_tree_description(context_tree_id):
    ...
    context_tree_description = {
        ...
        'setting_filename':
            context_tree_attributes['setting_file'].split('/')[-1],
    }
